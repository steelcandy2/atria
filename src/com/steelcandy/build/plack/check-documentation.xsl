...
<!-- TODO: !!!
    - check that there is a child element that isn't a 'space' or
      'attribute' element !!!
    - check that the first 'subconstruct' child element is not optional
      (that is, doesn't have 'number' attribute with value 'zero-or-more'
      or 'zero-or-one') if it is preceded only by child elements (terminals,
      flag-from-sets, etc.) that are optional
        - the parser code generator requires this, at least currently
-->
...
<!--
TODO: !!!
    - check that there is a child element that isn't a 'space' element !!!
    - check that the first 'subconstruct' child element is not optional
      (that is, doesn't have 'number' attribute with value 'zero-or-more'
      or 'zero-or-one') if it preceded only by child elements (terminals,
      flag-from-sets, etc.) that are optional
        - the parser code generator requires this, at least currently
-->
    ...
    <xsl:template match="compound-construct" mode="check">
        ...
        <!-- Check that the first subconstruct cannot be optional. -->
        <xsl:for-each select="subconstruct[1]">
            <xsl:if test="@number = 'zero-or-one' or
                          @number = 'zero-or-more'">
                <xsl:call-template name="report-error">
                    <xsl:with-param name="msg">
the first subconstruct of the '<xsl:value-of select="../@type"/>'
compound construct has a 'number' attribute that is
'<xsl:value-of select="@number"/>', which is invalid because the
first subconstruct of a compound construct cannot be optional.
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:if>
        </xsl:for-each>
...
is invalid.
    ...
    <xsl:template match="root-module | base-module | source-module |
                         tokens-module | constructs-module |
...
'<xsl:value-of select="$attribute-value"/>' - does.
    ...
    <xsl:template name="report-error">
        ...
        <xsl:message terminate="no">
At a node with pathname <xsl:value-of select="$pathname"/>: <xsl:value-of select="$msg"/>
        </xsl:message>
